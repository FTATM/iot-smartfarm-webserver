...
try {
  ...
  flush();
  updateScriptStatus('ai-decision', true, 'started', 1);


  // ============================================================
  // 2.โหลดไฟล์ที่จำเป็น config, function ต่างๆ
  // ============================================================
  updateScriptStatus('ai-decision', true, 'กำลังนำเข้าไฟล์ Config', 5);
  ...
  if (!$db) {
    ...
    flush();
    updateScriptStatus('ai-decision', false, 'ไม่สามารถเชื่อมต่อ database โปรดเช็ค logs.', 10);
    exit;
  }

  echo json_encode(["checkpoint" => 3, "msg" => "db ok"]) . "\n";
  flush();
  updateScriptStatus('ai-decision', true, 'กำลังเช็คความพร้อมของ AI', 10);
  ...
  if ($AI_MODE == 0) {
    ...
    flush();
    updateScriptStatus('ai-decision', false, 'ไม่รองรับการประมวลผล AI ในโหมดนี้', 12);

    $rawlog = INTO_log($db, $branch_id, $AI_MODE, $AI_MODEL, 'Decision', 0, "AI Decision is not available in Local mode.");

    file_put_contents(__DIR__ . '/into_log_result.txt', $rawlog);
    sleep(5);
    exit;
  }

  updateScriptStatus('ai-decision', true, 'กำลังดึงข้อมูลจากฐานข้อมูล', 15);
  ...
  if (empty($sensor_list['sensor'])) {
    ...
    flush();
    updateScriptStatus('ai-decision', false, 'ไม่พบอุปกรณ์ที่พร้อมใช้งาน โปรดเช็ค logs.', 37);
  ...
  flush();
  updateScriptStatus('ai-decision', true, 'เตรียมความพร้อมของข้อมูล', 37);
...
PROMPT;
  ...
  updateScriptStatus('ai-decision', true, "กำลังรอการตอบกลับจาก AI model planner", 41);
  // ================================================================
  ...
  if (isset($raw['success']) && !$raw['success'] && $raw['success'] == false) {
    ...
    flush();
    updateScriptStatus('ai-decision', false, "เกิดข้อผิดพลาดในการตอบกลับจาก AI planner", 50);
  ...
  flush();
  updateScriptStatus('ai-decision', true, "กำลังเรียกใช้งานเครื่องมือ", 50);
  ...
  if (isset($answer['required_tools']) && count($answer['required_tools']) == 0) {
    ...
    flush();
    updateScriptStatus('ai-decision', false, "เกิดข้อผิดพลาดในการเรียกใช้งานเครื่องมือ", 65);
  ...
  flush();
  updateScriptStatus('ai-decision', true, "เตรียมความพร้อมของข้อมูล", 65);
...
PROMPT;
  ...
  updateScriptStatus('ai-decision', true, "กำลังรอการตอบกลับจาก AI model decision", 65);
  // ================================================================
  ...
  if (isset($ai_decision_answer['success']) && !$ai_decision_answer['success'] && $ai_decision_answer['success'] == false) {
    ...
    updateScriptStatus('ai-decision', false, "เกิดข้อผิดพลาดในการตอบกลับจาก AI decision", 88);
    exit;
  }
  ...
  flush();
  updateScriptStatus('ai-decision', true, "กำลังเรียกใช้งานเครื่องมือ", 88);
  // ================================================================
  ...
  if (isset($response_set_output) && !$response_set_output['success']) {
    ...
    updateScriptStatus('ai-decision', false, 'เกิดข้อผิดพลาดในการเรียกใช้งานเครื่องมือ', 91);
    exit;
  }
  echo json_encode(["checkpoint" => 9, "msg" => "action executed"]) . "\n";
  flush();
  updateScriptStatus('ai-decision', true, "กำลังบันทึกการทำงาน", 91);
  ...
  updateScriptStatus('ai-decision', true, 'เสร็จสิ้นการทำงานใน branch [' . $branch_id . ']', 99);

  // เสร็จงาน
  updateScriptStatus('ai-decision', false, 'เสร็จสิ้น', 100);
} catch (Exception $e) {

  updateScriptStatus('ai-decision', false, 'เกิดข้อผิดพลาด : ' . $e->getMessage(), 0);
}
pg_close($db);
