<div id="popup-export-log" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm hidden">
    <div class="bg-white dark:bg-slate-900 w-full max-w-[25vw] rounded-[2rem] shadow-2xl border border-slate-200 dark:border-slate-800 flex flex-col overflow-hidden">
        <div class="px-8 py-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
            <h2 class="text-[1vw] font-bold font-display text-slate-900 dark:text-white">ส่งออกข้อมูล</h2>
            <button onclick="hidePopup('popup-export-log')" class="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors">
        ...
        <div class="p-8 space-y-6">
            ...
            <div>ส่งไปที่ Email</div>
            <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                <input id="email" class="col-span-3 w-full bg-slate-50 dark:bg-slate-800/50 border-slate-200 dark:border-slate-700 rounded-xl px-4 py-3 text-[0.75vw] focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all" placeholder="examble@email.com" type="text" />
                <button onclick="sentToEmail('popup-export-log')" class="px-5 py-2.5 rounded-xl text-[0.65vw] font-semibold text-white bg-primary hover:bg-orange-600 shadow-lg shadow-orange-500/20 transition-all font-display">
                    ส่ง
                </button>
            </div>
        ...
        <!-- <div class="px-8 py-6 bg-slate-50 dark:bg-slate-800/50 border-t border-slate-100 dark:border-slate-800 flex items-center justify-end gap-3">
            <button onclick="hidePopup('popup-export-log')" class="px-6 py-2.5 rounded-xl text-[0.65vw] font-semibold text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all font-display">
                ยกเลิก </br> (Cancel)
            </button>
            <button onclick="saveData('popup-export-log')" class="px-8 py-2.5 rounded-xl text-[0.65vw] font-semibold text-white bg-primary hover:bg-orange-600 shadow-lg shadow-orange-500/20 transition-all font-display">
                ส่งออก </br> (Export)
            </button>
        </div> -->
    </div>
</div>
<div id="popup-confirm-delete" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm hidden">
    <div class="bg-white dark:bg-slate-900 w-full max-w-[20vw] rounded-[2rem] shadow-2xl border border-slate-200 dark:border-slate-800 overflow-hidden text-center">
        <div class="pt-10 pb-6 px-8 flex flex-col items-center">
            ...
            <div class="flex flex-col sm:flex-row items-center justify-center gap-3 w-full pb-6">
                <button onclick="hidePopup('popup-confirm-delete')" class="w-full sm:w-auto px-8 py-3 rounded-xl text-[0.65vw] font-semibold text-slate-500 dark:text-slate-400 border-2 border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all font-display order-2 sm:order-1">
                    ยกเลิก </br> (Cancel)
                </button>
                <button onclick="deleteData(this,'popup-confirm-delete')" class="w-full sm:w-auto px-8 py-3 rounded-xl text-[0.65vw] font-semibold text-white bg-rose-500 hover:bg-rose-600 shadow-lg shadow-rose-500/25 transition-all font-display order-1 sm:order-2">
...
<div id="popup-export-result-add" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm hidden">
    <div
        ...
        <button onclick="hidePopup('popup-export-result-add')"
...
<div id="popup-export-result-delete" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-[2px] hidden">
    <div class="bg-white dark:bg-slate-900 w-full max-w-sm rounded-[2.5rem] shadow-2xl border border-slate-200 dark:border-slate-800 flex flex-col items-center p-10 text-center">
        ...
        <button onclick="hidePopup('popup-export-result-delete')"
            class="w-full py-3.5 px-8 rounded-2xl text-[0.65vw] font-semibold text-white bg-primary hover:bg-orange-600 shadow-lg shadow-orange-500/30 hover:shadow-orange-500/40 transition-all font-display">
            ตกลง </br> (OK)
        </button>
    </div>
...
<div id="popup-export-result-error" class="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm hidden">
    <div class="bg-white dark:bg-slate-900 w-full max-w-sm rounded-[2.5rem] shadow-2xl border border-slate-200 dark:border-slate-800 flex flex-col items-center p-10 text-center">
        ...
        <button onclick="hidePopup('popup-export-result-error')"
            class="w-full py-3.5 px-8 rounded-2xl text-[0.65vw] font-semibold text-white bg-primary hover:bg-orange-600 shadow-lg shadow-orange-500/30 hover:shadow-orange-500/40 transition-all font-display">
            ตกลง </br> (OK)
        </button>
    </div>
...
<?php include "../scripts/components/js-popup-export-log.html"; ?>