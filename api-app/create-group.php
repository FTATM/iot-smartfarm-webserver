...
$sql = "INSERT INTO page_data_manage_group ( group_name, sort, status, value_map_volte_censor, branch_id, create_time, update_time ) VALUES ($1,$2,$3,$4,$5,$6,$7)";

$now = date('Y-m-d H:i:s'); // แปลงเป็น string

// ✅ เตรียมค่าพารามิเตอร์
$params = [
    $decode->newname ?? "New Group",
    $decode->sort ?? 0,
    $decode->status ?? 1,
    $decode->Volte_map_volte_censor ?? 'Volte',
    $decode->branch_id ?? 1,
    $now,
    $now
];
